...
- **Memos and destination tags** — Append `#<memo>` to an address the way exchanges publish them: `rHb9…#12345` (XRP destination tag), Stellar and Hedera memos, Monero payment ids. The memo is part of the entry's identity, so two tags on one exchange account are separate entries, and it is stripped again before any explorer link or API lookup. Families with no memo concept reject the suffix rather than silently ignoring it
