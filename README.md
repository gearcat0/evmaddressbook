...
- **Import from xpub** — Paste an extended **public** key (xpub/ypub/zpub) to bulk-import its addresses as watch-only entries, with a preview, an optional on-chain activity probe, and per-address selection. Supports EVM, Bitcoin (legacy/nested/native SegWit), Dogecoin, Zcash transparent, Tron, and XRP. The key is used in the window only — **never stored, synced, or sent anywhere** — and extended *private* keys (xprv/yprv/zprv) are detected and refused. Solana, Cardano, and Monero cannot be derived from a BIP32 extended key and are excluded by design.
